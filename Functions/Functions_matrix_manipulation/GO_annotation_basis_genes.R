# GO annotation of the GEPs
GO_annotation_of_GEPs <- function(gene_clusters_file, 
                           ATgenes, 
                           store_dir = NULL, 
                           store_folder = "GO_annotation_of_GEPs",
                           use_ortho = FALSE,
                           ortho_table = NULL,
                           gene_column_in_use = NULL,
                           ortho_column_to_use = NULL){
  ...
  # check if the input is a loaded R object or a path containing file location
  if (is.character(gene_clusters_file) == TRUE){
    # Load the file containing the genes grouping for each factorization
    gene_clusters = loadRData(gene_clusters_file)
  }
  
  else {
    gene_clusters = gene_clusters_file
  }
  ...
  factorization_ID <- str_sub(colnames(gene_clusters), 3, -1) # Get the IDs/number of the factorization components, e.g. K = 3, 4, 5, 6, so on
  LF_ID <- colnames(gene_clusters) # Get the column names of gene_clusters / Basis components name
  
  for (i in c(1:ncol(gene_clusters))){
    ...
    if (!dir.exists(str_c(temp_dir, "Factorization_K_", factorization_ID[i]))){
      dir.create(str_c(temp_dir, "Factorization_K_", factorization_ID[i]), showWarnings = TRUE, recursive = FALSE, mode = "0777")
    }
    
    # Let's create a sub directory to store results for a particular factorization
    temp_nested_dir = str_c(temp_dir, "Factorization_K_", factorization_ID[i], "/")
    ...
    for (j in c(1:length(levels(gene_clusters[, temp_LF_ID])))){
      
      # get the gene ids in the current GEP j (set of coexpressed genes)
      GeneSet <- rownames(gene_clusters[gene_clusters[, temp_LF_ID] == j, , drop = FALSE])
      
      # Create a directory inside the previously created directory for the current GEP, j
      if (!dir.exists(str_c(temp_nested_dir, "GEP_", j))){
        dir.create(str_c(temp_nested_dir, "GEP_", j), showWarnings = TRUE, recursive = FALSE, mode = "0777")
      }
      
      temp_store_dir = str_c(temp_nested_dir, "GEP_", j, "/")
      
      # Save the set of genes in .txt file
      fileGenerator(GeneSet, fileName = str_c(temp_store_dir, "GEP_", j, "_genes.txt"))
      
      if (use_ortho == TRUE){
        tryCatch({
          ortho_table[, gene_column_in_use] = str_replace(ortho_table[, gene_column_in_use], "_", "-")
        }, 
        error = function(e){"Nothing to replace here"}
        )
        temp_ortho_ids = as.character(ortho_table[[gene_column_in_use]])
        temp_ortho_ids <- intersect(GeneSet, temp_ortho_ids)
        
        # Subsetting the data with the orthogenes only
        ortho_genes <- ortho_table[ortho_table[, gene_column_in_use] %in% temp_ortho_ids, ]
        ortho_genes <- as.character(ortho_genes[[ortho_column_to_use]])
        fileGenerator(ortho_genes, fileName = str_c(temp_store_dir, "GEP_", j, "_ortho_genes.txt"))
        GeneSet = ortho_genes
      }
      ...
      tryCatch({
        ...
        write.csv(GeneSet_all_BP, file = str_c(temp_store_dir, "GEP_", j, "_BP_all.csv"))
        ...
        write.csv(GeneSet_qurated_BP, file = str_c(temp_store_dir, "GEP_", j, "_significant_BP.csv"))
      }, 
      ...
      tryCatch({
        ...
        write.csv(GeneSet_all_MF, file = str_c(temp_store_dir, "GEP_", j, "_MF_all.csv"))
        
        GeneSet_ORA_MF_simplified <- clusterProfiler::simplify(GeneSet_ORA_MF)
        
        GeneSet_qurated_MF = GeneSet_ORA_MF_simplified@result
        
        write.csv(GeneSet_qurated_MF, file = str_c(temp_store_dir, "GEP_", j, "_significant_MF.csv"))
      }, 
      ...
      tryCatch({
        ...
        write.csv(GeneSet_all_CC, file = str_c(temp_store_dir, "GEP_", j, "_CC_all.csv"))
        
        GeneSet_ORA_CC_simplified <- clusterProfiler::simplify(GeneSet_ORA_CC)
        
        GeneSet_qurated_CC = GeneSet_ORA_CC_simplified@result
        
        write.csv(GeneSet_qurated_CC, file = str_c(temp_store_dir, "GEP_", j, "_significant_CC.csv"))
      }, 
      ...
      if (dim(GeneSet_ORA_BP)[1] < 1){
        ...
        ggsave(filename = str_c(temp_store_dir, "GEP_", j, "_all_BP_GO.png"), plot = p1, width = 12, height = 14, dpi = 300)
      }
      ...
      if (dim(GeneSet_ORA_BP_simplified)[1] < 1){
        ...
        ggsave(filename = str_c(temp_store_dir, "GEP_", j, "_sigificant_BP_GO.png"), plot = p2, width = 12, height = 14, dpi = 300)
      }
      ...
      if (dim(GeneSet_ORA_BP)[1] > 0){
        ...
        if (dim(GeneSet_ORA_BP@termsim)[1] == 1){
          ...
          tryCatch({
            ...
            ggsave(filename = str_c(temp_store_dir, "GEP_", j, "_all_BP_GO_modules_network.png"), plot = p3, width = 12, height = 14, dpi = 300)
          }, 
      ...
      if (dim(GeneSet_ORA_BP_simplified)[1] > 0){
        ...
        if (dim(GeneSet_ORA_BP_simplified@termsim)[1] == 1){
          ...
          tryCatch({
            ...
            ggsave(filename = str_c(temp_store_dir, "GEP_", j, "_significant_BP_GO_modules_network.png"), plot = p4, width = 12, height = 14, dpi = 300)
          }, 
          error = function(e){"No module found"}
          )
        }
