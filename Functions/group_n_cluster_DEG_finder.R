# Identify differentially expressed genes for a group/replicate/species in each cluster
group_n_cluster_DEG_finder <- function(seuratObject,
  ...
  if (!dir.exists(str_c(store_dir, "/", store_folder, "/", grouping_variable, "_and_cluster_DEGs"))){
    dir.create(str_c(store_dir, "/", store_folder, "/", grouping_variable, "_and_cluster_DEGs"), showWarnings = TRUE, recursive = FALSE, mode = "0777")
  }
  
  if (!dir.exists(str_c(store_dir, "/", store_folder, "/", grouping_variable, "_and_cluster_DEGs", "/", grouping_variable, "_and_cluster_DEtest_", DEGtest))){
    dir.create(str_c(store_dir, "/", store_folder, "/", grouping_variable, "_and_cluster_DEGs", "/", grouping_variable, "_and_cluster_DEtest_", DEGtest), showWarnings = TRUE, recursive = FALSE, mode = "0777")
  }

  # storing the directory information in a temporary variable
  temp_dir = str_c(store_dir, "/", store_folder, "/", grouping_variable, "_and_cluster_DEGs", "/", grouping_variable, "_and_cluster_DEtest_", DEGtest, "/")
  ...
  seuratObject[["combined_identity"]] <- factor(str_c(Idents(seuratObject), seuratObject@meta.data[[grouping_variable]], sep = "_"))
  ...
  for (i in c(1:length(temp_pattern))){
    ...
    # What are the groups in each cluster
    ...
    for (j in c(1:length(groups_per_cluster))){
      ...
      if (temp_cell_per_cluster[groups_per_cluster[j], ] < min_cell_for_DE){
        ...
        cat(temp_text, file = str_c(temp_dir, "No_markers_cell_C", temp_str[2], "_", temp_str[1], ".txt"))
      }
      else {
        group_marker = FindMarkers(seuratObject, ident.1 = groups_per_cluster[j], test.use = DEGtest, min.pct = min.pct, logfc.threshold = lfc, only.pos = Positive_exp)
        temp_str = c(gsub(pattern = "[^[:alnum:] ]", replacement = "", x = str_split(groups_per_cluster[j], pattern = "_")[[1]][2]), str_split(groups_per_cluster[j], pattern = "_")[[1]][1])
        do.call("<-", list(str_c("Cluster_", temp_str[2], "_", temp_str[1]), group_marker))
        save(list = str_c("Cluster_", temp_str[2], "_", temp_str[1]), file = str_c(temp_dir, "Cluster_", temp_str[2], "_", temp_str[1], "_DEGs.RData"))
      }
    }
  }
}
