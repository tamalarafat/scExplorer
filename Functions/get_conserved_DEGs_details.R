get_conserved_DEGs_details <- function(DEG_file_dir, 
  ...
  for (i in c(1:length(DEG_files))){
    ...
    All_DEGs_description = merge(Cluster_DEGs, description, by.x = "gene_ID", by.y = "CH_ID", all.x = TRUE)
    ...
    CS_DEGs_description = merge(Cluster_specific_DEGs, description, by.x = "gene_ID", by.y = "CH_ID", all.x = TRUE)
